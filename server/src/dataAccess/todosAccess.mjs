...
export class TodoAccess {
  ...
  async getAllTodos(userId) {
    logger.info(`Getting all todos by user_id ${userId}`, {
  ...
  async createTodo(todo) {
    logger.info(`Create a new todo: ${todo}`, { function: "createTodo()" });
  ...
  async updateTodo(updatedTodoItem) {
    logger.info(`Updating a todo item: ${updatedTodoItem}`, {
      function: "updateTodo()",
    });

    const newUpdatedTodoItem = await this.dynamoDbClient.update({
      TableName: this.todosTable,
      Key: {
  ...
  async deleteTodo(deletedTodo) {
    logger.info(`Deleting a todo item: ${deletedTodo}`, {
      function: "deleteTodo()",
    });
  ...
  async updateTodoAttachmentUrl(updatedTodoAttachment) {
    logger.info(`Updating a todo Attachment ${updatedTodoAttachment} `, {
      function: "updateTodoAttachmentUrl()",
    });
